...
import { updateTaskStatus } from "@/app/dashboard/teams/actions"
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar"
import { Button } from "@/components/ui/button"
import { Calendar, MessageSquare, MoreHorizontal, Crosshair, Target, CheckCircle2, Zap } from "lucide-react"
import { format } from "date-fns"
import { cn } from "@/lib/utils"
import { Badge } from "@/components/ui/badge"
import { StatusBadge } from "@/components/ui/status-badge"

// Extended Task type to include assignees and team
type TaskWithAssigneesAndTeam = Task & {
    assignees: {
        id: string
        name: string | null
        image: string | null
...
interface AllTasksKanbanProps {
    initialTasks: TaskWithAssigneesAndTeam[]
}
...
export function AllTasksKanban({ initialTasks }: AllTasksKanbanProps) {
...
function KanbanColumn({ id, title, tasks, count, color, icon: Icon }: { id: TaskStatus; title: string; tasks: TaskWithAssigneesAndTeam[], count: number, color: string, icon: React.ElementType }) {
...
function DraggableTaskCard({ task }: { task: TaskWithAssigneesAndTeam }) {
...
function TaskCard({ task }: { task: TaskWithAssigneesAndTeam }) {
        ...
        <div className="bg-card border border-border rounded-xl p-4 shadow-sm hover:shadow-md transition-shadow cursor-grab active:cursor-grabbing group">
            <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {task.team && (
                        <Badge variant="outline" className="text-[10px] px-1.5 py-0.5 h-auto font-normal max-w-[100px] truncate block" title={task.team.name}>
                            {task.team.name}
                        </Badge>
                    )}
